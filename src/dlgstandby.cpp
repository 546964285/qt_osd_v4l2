...
DlgStandby::DlgStandby(QWidget *parent) :QDialog(parent),ui(new Ui::DlgStandby)
{
    ui->setupUi(this);
...
void DlgStandby::SlotFreshThisWindow()
{
    ReadiniRemember();//读取ini文件
    ControlInit();//控件初始化
}

void DlgStandby::SlotTranslateThisWindow()
{
    ReadiniRemember();//读取ini文件
    ControlInit();//控件初始化
}

void DlgStandby::keyPressEvent(QKeyEvent *event)
{
    qDebug() << "DlgStandby Pressed Key is" << QKeySequence(event->modifiers() + event->key());
    key_flg=false;
    emit SKeyPressed();
    switch (event->key())
    {
...
void DlgStandby::keyReleaseEvent(QKeyEvent *event)
{
    qDebug() << "DlgStandby pressed key is" << QKeySequence(event->modifiers() + event->key());
    switch (event->key())
    {
        case Qt::Key_Return:
        {
            if(m_RowNum==0)//处于5分钟未操作，待机
            {
                hide();
                emit SShowdlgf1();
                WriteiniRemember("5");//将待机时间设置存入ini文件
            }
            else if(m_RowNum==1)//处于10分钟未操作，待机
            {
                hide();
                emit SShowdlgf1();
                WriteiniRemember("10");//将待机时间设置存入ini文件
            }
            else if(m_RowNum==2)//处于15分钟未操作，待机
            {
                hide();
                emit SShowdlgf1();
                WriteiniRemember("15");//将待机时间设置存入ini文件
            }
        }
        break;
        case Qt::Key_M:
...
void DlgStandby::WriteiniRemember(QString  StrStandby)//将待机时间设置存入ini文件
{
    //创建或者打开指定路径下的.ini文件
    QSettings *ConfigIniWrite = new QSettings("./qt_osd_arm.ini", QSettings::IniFormat);
    ConfigIniWrite->setValue("standby/set1", StrStandby);//standby   下的set1内容设置
    delete ConfigIniWrite;//写入完成后删除指针
}

void DlgStandby::ReadiniRemember()//读取ini文件
{
    QSettings   *ConfigIniRead   =    new QSettings("./qt_osd_arm.ini", QSettings::IniFormat);
    QString     StrStandbySet1   =    ConfigIniRead->value("standby/set1").toString();
    qDebug() <<"StrStandbySet1   is"  << StrStandbySet1;
    if(StrStandbySet1=="5")
    {
        m_RowNum=0;
    }
    else if(StrStandbySet1=="10")
    {
        m_RowNum=1;
    }
    else if(StrStandbySet1=="15")
    {
        m_RowNum=2;
    }
    else
...
void DlgStandby::ControlInit()//控件初始化
{
    QFont    Font;
    QPalette Palette;
    ...
    QString Setting =tr("Standby");//字  拍照设置
...
void DlgStandby::LabelSet1_Show()
{
...
void DlgStandby::LabelSet2_Show()
{
    Label123Clear();//清除Lable123内容
...
void DlgStandby::LabelSet3_Show()
{
    Label123Clear();//清除Lable123内容
...
void DlgStandby::Label123Change()
{
